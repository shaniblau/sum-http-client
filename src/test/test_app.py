...
def test_handle_half_second_half_should_call_load_and_handle_second_half(app_fixture, mocker):
...
def test_handle_second_half_2_identical_files_names_should_log_error_and_call_handle_file(app_fixture, mocker, caplog):
    ...
    assert expected_error == error_messages[0]
    assert len(error_messages) == 1
    mock_handle_file.assert_called_once_with('file_a.txt', 'file')
